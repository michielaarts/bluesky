...
class ScenarioGenerator:
    ...
    def write_scenario(
        ...
        if len(all_scen) > 1:
            ...
            for filename in [filename_nr, filename_wr]:
                with open(filepath / filename, 'w') as f:
                    f.write('# ########################################### #\n')
                    f.write(f'# Batch file {filename} for files:\n')
                    ...
                    for i in range(len(all_scen)):
                        t = all_scen[i]['duration'] * safety_factor
                        f.write(f'{self.tim2txt(i * t)}>PCALL URBAN/{all_filenames[i]} REL\n')
                        if 'WR' in filename:
